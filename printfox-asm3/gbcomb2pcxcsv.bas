...
3 REM 0) USES A CSV FILE TO KNOW ABOUT DISCIPLINES, NAMES AND RANKS
4 REM 1) BLOAD GB FILE (SPECIAL TREATMENT FOR FIRST 2 BYTES)
5 REM 2) UNPACKS GB FILE BY USING GB2VDC.BIN (IE RE-ORGANIZING BYTES TO BE CONTINOUS)
6 REM 3) PACKS RAW DATA FROM STEP 2 TO PCX FORMAT BY USING PCXPACK.BIN
7 REM 4) FSAVE PACKED PCX-FILE
...
#  DD:DATA-DISK (CONTAINS BINARIES AND CSV FILES)
13 DD=PEEK(186)
14 INPUT "READ INPUT FILES (GB1 AND GB2 FILES) FROM #";I$:ID=VAL(I$)
15 INPUT "WRITE OUTPUT FILES (PCX) TO #";I$:OD=VAL(I$)
...
32 DIM IM(DM),PN$(DM,PM),PS(DM,PM),PR%(DM,PM),SO(DM,PM),SO$(DM,PM),TS(DM,PM),DN$(DM),MP$(PM)


33 DIM FI$(99),DT$(50,3)
...
39 PRINT FRE(0) FRE(1) 

40 FI$="WERTNG"+CI$+DI$+".CSV"
41 DOPEN #1,(FI$),U(DD):IF DS<>0 THEN PRINT DS$" FILE "FI$:END
42 X=0:DO 
...
100  FI$=CI$+"-"+DN$+"-"+PN$+".GB1"
101  DOPEN#1,(FI$),R,U(ID):CS=DS:DCLOSE#1
102  IF CS=62 THEN PRINT FI$" MISSING. SKIPPING.":MP=MP+1:MP$(MP)=DT$(R,3):GOTO150
...
#    # LOAD GB2 FILE
121  FI$=CI$+"-"+DN$+"-"+PR$+".GB2"
122  DOPEN#1,(FI$),R,U(ID):CS=DS:DCLOSE#1
...
2040 BLOAD (FI$),B(SB),P(SA+2),U(ID)
2041 PRINT "DONE"
