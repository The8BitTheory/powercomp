...
#######################################################
# THIS EXTRACTS DATA FROM CSV FILE AND WRITES IT INTO #
# TF FILES FOR PRINTFOX
#
# 
#######################################################
...
26 DIM IM(DM),PN$(DM,PM),PS(DM,PM),D$(2)
...
160 PRINT FRE(0) FRE(1):DIM DT$(50,3)
164 CF$="WERTNG"+CI$+DI$+".CSV"
165 PRINT "LOADING "CF$

170 OPEN 1,DD,0,CF$
172 IF DS<>0 THEN PRINT DS""DS$" FILE "CF$:CLOSE1:END
180 X=0:DO

190  INPUT#1,DT$(X,0),DT$(X,1),DT$(X,2)
#191  SP=INSTR(DT$(X,0),";"):IF SP=0 THEN PRINT"NO SEMICOLON IN "DT$(X,0):GOTO205

# FIX IGNORED SEMICOLONS AS SEPARATORS
#192  Y=X
#193  D$(0)=DT$(X,0):D$(1)=DT$(X,1):D$(2)=DT$(X,2)

#194  S1=SP:S2=INSTR(DT$(X,0),";",S1+1):S3=INSTR(DT$(X,0),";",S2+1)
#195  DT$(X,0)=LEFT$(D$(0),S1-1):DT$(X,1)=MID$(D$(0),S1,S2-1):DT$(X,2)=MID$(D$(0),S2)
#196  IF LEFT$(DT$(X,0),1)=CHR$(10) THEN DT$(X,0)=MID$(DT$(X,0),2)
#197  X=X+1:IF X-Y<3 THEN 194
...
500 OPEN1,DD,1,"@0:"+FI$
501 IF DS<>0 THEN PRINT DS$" FILE "FI$:CLOSE1:END
# WRITE
#  START OF FILE (84)
#  FORMATLINE (02)
