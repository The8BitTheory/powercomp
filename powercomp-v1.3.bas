...
16 BLOAD "MOUSE128.BIN",U(DU):SYS DEC("1800")
...
33 CX=0:CM=0:A$="":B$="":L$="":H$="":EP$=CHR$(0):CC=0:PU=0:CI=0:SP=-1:DT$="":DT=-1:DC=-1:Y1=630:Y2=650
35 TR%=0:SL$="":TC$="":TD$="":PG$="":Y1$="":Y2$="":TR$="":FX=1:WW%=0:WH%=0:MN%=-1:MI%=0



100 DIM PN$(DM,PM),PS(DM,PM),PR%(DM,PM),SO(DM,PM),SO$(DM,PM),TS(DM,PM),DN$(DM),IM(DM),CP(6),CL(6),P(6),RC(PM)
101 DIM CE$(143),PU$(4),NN$(PM),TL$(9),WX%(3),W%(1,3),BD$(2),MN%(2,1),MN$(2,6),MS%(2,1),MU$(2)
105 GOSUB 63000
...
283 IF MN%>=0 THEN GOSUB 55000:GOTO 410
...
322  IF I$="" THEN BEGIN
323   IF LEN(CN$)=0 THEN CI=3:P=2:ELSE CI=0:P=PI+4
324   AT=-1
325  BEND
...
329 BEND:IF AT THEN AT=0:SC=-1:GOTO 399
...
#   SAVE DATAFILE NAME
332 IF I$=CHR$(13) THEN IF CI=5 THEN GOSUB703:GOSUB 1000:DC=-1:CI=0:PI=0:P=PI+4:GOTO399
...
# COMMODORE +F FOR FILE MENU
336 IF I$=CHR$(187) THEN IF INSTR(SM$,CHR$(CI))>0 THEN MN%=0:GOSUB 61000:GOTO410

# COMMODORE +O FOR ORDER MENU
337 IF I$=CHR$(185) THEN IF INSTR(SM$,CHR$(CI))>0 THEN MN%=1:GOSUB 61000:GOTO410

# COMMODORE +G FOR GENERATE MENU
338 IF I$=CHR$(165) THEN IF INSTR(SM$,CHR$(CI))>0 THEN MN%=2:GOSUB 61000:GOTO410




# F2:COMPETITION NAME
340 IF ASC(I$)=137 THEN GOSUB 703:CI=3:P=2:GOSUB 705:GOTO 410

# F4:DISCIPLINES
341 IF ASC(I$)=138 THEN GOSUB 703:CI=2:P=CD+4:GOSUB 705:GOTO 410
...
346  IF LEN(DN$)=0 THEN GOSUB2100:GOTO399:ELSE GOSUB 1000:GOTO 399
347 BEND
...
# ORDER BY ID
394 IF ASC(I$)=133 THEN T$="ORTING BY ":GOSUB1120:GOSUB 1200:GOTO 399
...
395 IF ASC(I$)=134 THEN GOSUB 1300:GOTO 399

# ORDER BY PLAYER SCORE
396 IF ASC(I$)=135 THEN GOSUB 1400:GOTO 399

# REVERSE ORDER OF CURRENT LIST
397 IF ASC(I$)=136 THEN GOSUB 1500:GOTO 399
...
1101 GOSUB703:CI=4:SC=0:CC=0:P=CC+7:REM WINDOW 10,2,70,22,1:COLOR5,13

1103 W%=0:T$="OAD ILE":GOSUB 60000


1110 COLOR 5,1
1112 CHAR 1,1,19,"  : ANCEL       ETURN: OAD ILE       : ENAME ILE   "
...
1900 IF PR%(CD,0)=0 THEN GOSUB 1400

1901 T$="HOWING SCOREBOARD ON 40 COL DISPLAY":GOSUB 1120
...
1998 FAST:PRINT"":GRAPHIC5:COLOR 5,2
1999 RETURN
...
29810 BLOAD "CHARSET.10",B0,P(BE)
29820 RTV BE,8192,8192
29830 RETURN
...
55000 IF ASC(I$)=95 THEN MN%=-1:GOSUB 1700:GOTO 55099
...
#10,2,70,22
60000 GOSUB 1600
60002 WINDOW W%(W%,0),W%(W%,1),W%(W%,2),W%(W%,3),1
...
61025 WINDOW MN%(MN%,0),2,MN%(MN%,0)+TL%+1,2+IX+1,1
...
61100 VMF 2048+241+MN%(MN%,0)+MI%*80,192,MS%(MN%,0):RETURN

# DE-HIGHLIGHT MENU ENTRY
61110 VMF 2048+241+MN%(MN%,0)+MI%*80,128,MS%(MN%,0):RETURN



# DECLARE MENU STRUCTURE
63000 MN$(0,0)="EW":MN$(0,1)="PEN":MN$(0,2)="AVE":MN$(0,3)="AVE AS...":MN$(0,4)="-":MN$(0,5)="XIT"
63002 MN$(1,0)="BY ":MN$(1,1)="BY AME":MN$(1,2)="BY CORE"
63003 MN$(2,0)="COREBOARD":MN$(2,1)="RINTFOX ":MN$(2,2)=""
...
# TOP BORDER
63110  TR$=CHR$(142)+""+LEFT$(BD$(2),TL%)+""

63112  FOR L=0 TO IX-1
63114   IF MN$(MN%,L)="-" THEN TR$=TR$+CHR$(142)+""+LEFT$(BD$(2),TL%)+"":ELSE BEGIN
