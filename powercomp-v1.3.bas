...
12 FAST:GRAPHIC5:DU=PEEK(186):BE=PEEK(4624)+PEEK(4625)*256
13 BLOAD "VDCBASIC2D.0AC6",U(DU):SYS DEC("AC6"):RST3:PRINT CHR$(14);:VMF 0,102,2000

14 COLOR4,15:COLOR0,7:COLOR5,13:COLOR6,16:POKE 828,183:POKE2603,104:GOSUB63900


15 GOSUB 29800:REM GOSUB 29900

#16 CHAR 1,20,11,"LOADING MOUSEDRIVER...":BLOAD "MOUSE128.BIN",U(DU):SYS DEC("1800")


16 CHAR 1,20,11,"INITIALIZING VARIABLES AND ARRAYS...     "
...
#PO:PLAYER LIST OFFSET (FOR SCROLLING). GOES FROM 0 TO IM(CD)-18
#PB:PLAYER BAR LINE (SCREEN LINE AND PLAYER ID). GOES FROM 0 TO 17
...
#VP:MAXIMUM PLAYER BAR LOCATION (IMPORTANT FOR CASES WHERE LESS THAN 18 PLAYERS ARE LISTED)
32 PM=31:DM=6:IM=0:DF=0:FD=0:TS=0:PI=0:CN$="":CD=0:SC=-1:DN$="":CF$="":T$="":PC=0:PW=0:AT=0:EA=0:TX=0:PO=0:PB=0:VP=0:T2$=""
...
#NW:NEW
#C$:USED AS A TEMPORARY WORKING VARIABLE
33 CX=0:CM=0:A$="":B$="":L$="":H$="":EP$=CHR$(0):CC=0:CO=0:PU=0:CI=0:SP=-1:DT$="":DT=-1:DC=-1:Y1=630:Y2=650
35 TR%=0:SL$="":TC$="":TD$="":PG$="":Y1$="":Y2$="":TR$="":FX=1:WW%=0:WH%=0:MN%=-1:MI%=0:NW=0:IX=0:PN$="":C$=""



100 DIM PN$(DM,PM),PS(DM,PM),PR%(DM,PM),SO(DM,PM),SO$(DM,PM),TS(DM,PM),DN$(DM),IM(DM),CP(8),CL(8),P(8),RC(PM)
101 DIM CE$(143),PU$(4),NN$(PM),TL$(9),W%(3,3),BD$(2),MN%(2,1),MN$(2,6),MS%(2,1),MU$(2)


103 CHAR 1,20,11,"LOAD MENU-STRUCTURE"+LEFT$(SP$,19)

105 GOSUB 63000
#105 GOSUB 62900

107 CHAR 1,20,11,"SOME MORE VARIABLES AND ARRAYS"+LEFT$(SP$,9)
...
# W%2 = CATALOG LIST AREA IN FILE DIALOG (WITHOUT HEADER AND FOOTER)
# W%3 = PROGRESS DIALOG
134 W%(0,0)=10:W%(0,1)=2:W%(0,2)=70:W%(0,3)=22
135 W%(1,0)=2 :W%(1,1)=4:W%(1,2)=39:W%(1,3)=23
136 W%(2,0)=W%(0,0)+6:W%(2,1)=W%(0,1)+3:W%(2,2)=W%(2,0)+27:W%(2,3)=W%(2,1)+9
137 W%(3,0)=20:W%(3,1)=8:W%(3,2)=60:W%(3,3)=12


138 CHAR 1,20,11,"CALCULATE MENU SIZES"+LEFT$(SP$,20)

#   CALCULATE MENU SIZES
139 GOSUB 63100

# LOAD SCREEN FROM DISK
140 CHAR 1,20,11,"LOAD SCREEN FROM DISK"+LEFT$(SP$,19):GOSUB 1650
...
# CHECK FOR STATEFILE
142 CHAR 1,20,11,"CHECK FOR STATEFILE"+LEFT$(SP$,20):GOSUB 32000
...
146  CI=0:PI=0:P=PI+4:PB=0:PO=0
...
#   SET SORT ORDER TO ID, RECOVER SCREEN, WRITE DISCIPLINES, SHOW CURSOR  BAR
155 GOSUB 1200:GOSUB1700:GOSUB 800:GOSUB 1800:GOSUB 705
...
230 IF MN%>=0 THEN GOSUB 55000:GOTO 400
...
# NUMERIC
284 IF ASC(I$)>47 THEN IF ASC(I$)<58 THEN IF INSTR(NI$,CHR$(CI))>0 THEN GOSUB 540:GOTO 400
# ALPHABETICAL LOWER CASE
285 IF ASC(I$)>64 THEN IF ASC(I$)<91 THEN IF INSTR(AI$,CHR$(CI))>0 THEN GOSUB 540:GOTO 400
# ALPHABETICAL UPPER CASE
286 IF ASC(I$)>192 THEN IF ASC(I$)<219 THEN IF INSTR(AI$,CHR$(CI))>0 THEN GOSUB 540:GOTO 400

# DOWN
290 IF I$=CHR$(17) THEN IF INSTR(C1$,CHR$(CI))>0 THEN GOSUB 420:GOTO 400
# UP
291 IF I$=CHR$(145) THEN IF INSTR(C1$,CHR$(CI))>0 THEN GOSUB 470:GOTO 400
# RIGHT
292 IF I$=CHR$(29) THEN IF INSTR(C2$,CHR$(CI))>0 THEN GOSUB 630:GOTO 400
# LEFT
293 IF I$=CHR$(157) THEN IF INSTR(C2$,CHR$(CI))>0 THEN GOSUB 670:GOTO 400
...
301 IF I$=" " THEN GOSUB 540:GOTO 400
302 IF I$="-" THEN GOSUB 540:GOTO 400
303 IF I$="." THEN GOSUB 520:GOTO 400
...
340 IF I$=CHR$(13) THEN IF CI=2 THEN GOSUB703:CI=0:PI=0:PB=0:PO=0:P=PI+4:GOSUB1200:GOTO 399

#   SAVE COMPETITION NAME
341 IF I$=CHR$(13) THEN IF CI=3 THEN IF LEN(T$)>0 THEN GOSUB703:CI=0:PI=0:PB=0:PO=0:P=PI+4:GOSUB705:GOTO 400

#   SAVE DATAFILE NAME
342 IF I$=CHR$(13) THEN IF CI=5 THEN GOSUB703:GOSUB 1000:DC=-1:CI=0:PI=0:PB=0:PO=0:P=PI+4:GOTO399

# CONFIRM RENAMING FILE
343 IF I$=CHR$(13) THEN IF CI=6 THEN GOSUB 2500:GOTO 400
...
# COMMODORE +1
350 IF I$=CHR$(129) THEN GOSUB703:CD=0:PI=0:PB=0:PO=0:P=PI+4:GOSUB1200:GOTO 399
# COMMODORE + 2-6
351 IF ASC(I$)>148 THEN IF ASC(I$)<155 THEN GOSUB703:CD=ASC(I$)-148:PI=0:PO=0:PB=0:P=PI+4:GOSUB1200:GOTO 399



# CONTROL +F FOR FILE MENU
352 IF I$=CHR$(6) THEN IF INSTR(SM$,CHR$(CI))>0 THEN MN%=0:GOSUB 61000:GOTO400

# CONTROL +O FOR ORDER MENU
353 IF I$=CHR$(15) THEN IF INSTR(SM$,CHR$(CI))>0 THEN MN%=1:GOSUB 61000:GOTO400

# CONTROL +G FOR GENERATE MENU
354 IF I$=CHR$(7) THEN IF INSTR(SM$,CHR$(CI))>0 THEN MN%=2:GOSUB 61000:GOTO400



# F2:COMPETITION NAME
355 IF ASC(I$)=137 THEN GOSUB 703:CI=3:P=2:GOSUB 705:GOTO 400

# F4:DISCIPLINES
356 IF ASC(I$)=138 THEN GOSUB 703:CI=2:P=CD+4:GOSUB 705:GOTO 400
...
359 IF I$=CHR$(165) THEN AC=1:GOSUB1600:GOSUB1100:GOTO400

# COMMODORE+P - PREVIEW
360 IF I$=CHR$(175) THEN GOSUB 1900:GOTO 399

# COMMODORE+M - MASS-AUTOMATION EXPORT
361 IF I$=CHR$(167) THEN GOSUB 2600:GOTO 399


# COMMODORE+S FOR SAVE
362 IF ASC(I$)=174 THEN BEGIN
363  IF LEN(DN$)=0 THEN GOSUB 2100:GOTO399:ELSE GOSUB 1000:GOTO 399
364 BEND

# COMMODORE+A FOR SAVE AS...
365 IF ASC(I$)=176 THEN GOSUB 2100:GOTO 399

# COMMODORE+L FOR LOAD
366 IF ASC(I$)=182 THEN AC=0:GOSUB1600:GOSUB 1100:GOTO 400



# CBM+N - NEW COMPETITION
367 IF I$=CHR$(170) THEN NW=-1:GOTO 400

# CBM+X - EXIT PROGRAM
368 IF I$=CHR$(189) THEN 55350



# ORDER BY ID
369 IF ASC(I$)=133 THEN T$="ORTING BY ":GOSUB1120:GOSUB 1200:GOTO 399

# ORDER BY PLAYER NAME
# 1300:SORT ALPHABETICALLY
370 IF ASC(I$)=134 THEN GOSUB 1300:GOTO 399

# ORDER BY PLAYER SCORE
371 IF ASC(I$)=135 THEN GOSUB 1400:GOTO 399

# REVERSE ORDER OF CURRENT LIST
372 IF ASC(I$)=136 THEN GOSUB 1500:GOTO 399
...
## PUT NOTHING IN BETWEEN THESE TWO LINES (399 AND 400)
400 LOOP UNTIL NW

415 SCRATCH "POWERCOMP.STATE",U(DU):CLR:GOTO 17
...
423 GOSUB 703
#424 PI=PI+1:IF PI>17 THEN PO=PO+1:IF PO>IM(CD)-PO THEN PO=IM(CD)-PO
424 PB=PB+1:IF PB > 17 THEN BEGIN

#    CHECK IF SCROLL (HIGHEST PLAYER-ID IS LARGER THAN SELECTION-BAR PLUS SCROLL-OFFSET), OR ROLL-OVER TO TOP
#             +1 BECAUSE WE ALSO NEED THE "NEW PLAYER" LINE
425  IF IM(CD)+1>PB+PO THEN PO=PO+1:PB=17: ELSE PO=0:PB=0

426 BEND:GOSUB800

427 P=PB+4:PI=PO+PB

429 GOSUB 705:RETURN
...
474 PB=PB-1:IF PB<0 THEN GOSUB 500:GOSUB 800

478 P=PB+4:PI=PO+PB

479 GOSUB 705:RETURN
...
# SCROLL
500  IF PO=0 THEN BEGIN
505   VP=IM(CD)+1:IF VP>17 THEN VP=17
508   PO=IM(CD)+1-VP:PB=VP
509  BEND:GOTO 515


#    SCROLL UP?                          OR SCROLL TO BOTTOM
510  IF PO>0 THEN PO=PO-1:PB=0
...
#   DISPLAY IDS 0-17 PLUS PLAYER OFFSET (EG 1-18 WHEN PLAYER-OFFSET IS 1)
804 VP=IM(CD):IF VP>17+PO THEN VP=17+PO
805 FOR L=PO TO VP
# IDS
806  VL=SO(CD,L)
...
1122 GOSUB 703:W%=3:WINDOW W%(W%,0),W%(W%,1),W%(W%,2),W%(W%,3),1
1124 PRINTCHR$(27)"M";
1125 PRINT"";
1126 CHAR 1,0,1
1127 PRINT"";
...
1600 VMC 0,4096,4096
1606 VTR 4096,BE,4096
#1608 BSAVE "SCREEN",P(BE) TO P(BE+4096),U(DU),B0
1610 RETURN

# LOAD SCREEN FROM DISK
1650 REM IF VMD(25)=80 THEN GOTO 1700
1652 CHAR 1,20,11,"LOADING SCREEN..."+LEFT$(SP$,13):BLOAD"SCREEN",P(BE),B0,U(DU)
1654 RTV BE,4096,4096
1655 RETURN

# COPY OFF-SCREEN TO SCREEN
1700 VMC 4096,0,4096
1710 RETURN



# WRITE LIST OF DISCIPLINES
1800 WINDOW 42,4,78,12,1:COLOR 5,1
1805 FOR L=0 TO DX
...
1902 CHAR 1,2+W%(3,0),2+W%(3,1),"RESS  TO CHANGE NR OF ECIMALS"
1903 CHAR 1,2+W%(3,0),3+W%(3,1),"NY OTHER KEY CLOSES COREBOARD"
...
##########################
# EXPORT AUTOMATION FILE #
##########################

2600 T$="XPORTING FOR UTOMATION":GOSUB1120

#    T2$:4 LETTERS OF THE DISCIPLINE NAME. ALL SAME CASE.
2605 T2$=LEFT$(DN$(CD),4)

2610 T=ASC(LEFT$(T2$,1))
2612 IF T>128 THEN T2$=CHR$((T-96) AND (255-32))+MID$(T2$,2)

2614 CF$="S24-"+T2$+".AUTO,S,W"

2616 OPEN1,DU,2,"@0:"+CF$

2618 IX=0:PW=38/IM(CD):X=L+2
2620 FOR L=0 TO IM(CD)

2622  T$=LEFT$(PN$(CD,SO(CD,L)),3):
#2624  IF LEN(TS$)<3 THEN 2720

#     PN$:3 LETTERS OF PLAYER NAME. ALL SAME CASE
2626  PN$=""
2628  FOR I=1 TO LEN(T$)
2630   T=ASC(MID$(T$,I,1))
2632   IF T>128 THEN C$=CHR$((T-96) AND (255-32)):ELSE C$=MID$(T$,I,1)
2634   PN$=PN$+C$
2636  NEXT

#     T$: THE MEMORY-SLOT INDEX
2638  T$=MID$(STR$(X),2)

# TODO: INTRODUCE ID FOR COMPETITION (USED AS A FILENAME PREFIX)
# PARAMETER SPEC:
# 1: COMMAND (G0,UP,G+,UO,FS,PH,PD,FS)
# 2: FILENAME (IF NO FILE READ OR WRITE IS INVOLVED, USE -)
# 3: READ FROM BANK
# 4: READ FROM ADDRESS-SLOT
# 5: WRITE TO BANK
# 6: WRITE TO ADDRESS-SLOT

# ADDRESS SLOTS MAP START- AND END-ADDRESS TO A GIVEN ID

#     LOAD TOP-HALF GB FILE TO B1,VE (1,0)
2640  PRINT#1,"G0,S24.GB1,-,-,1,0"
#     UNPACK TO B0,BE (0,0)
2642  PRINT#1,"UN,-,1,0,0,0"

#     LOAD DISCIPLINE GB FILE
2644  PRINT#1,"G+,DISZ"+T2$+".GB,-,-,1,1"
#     MERGE TO B0,BE
2646  PRINT#1,"UO,-,1,1,0,0"

#     LOAD PLAYER-NAME GB FILE
2648  PRINT#1,"G+,SPI"+PN$+".GB,-,-,1,"+T$
#     MERGE TO B0,BE
2650  PRINT#1,"UO,-,1,"+T$+",0,0"

#     PACK GB-FILE
2652  PRINT#1,"PG,-,0,0,0,1"
#     SAVE FILE TO DISK
2654  PRINT#1,"FS,S24-"+T2$+"-"+PN$+".GB1,0,1,-,-"



#2670  TS=PS(CD,SO(CD,L))
#2680  T$=MID$(STR$(IX),2)+";"+TS$+";"+MID$(STR$(TS),2)
#2690  GOSUB 2300:PRINT#1,T2$CHR$(13)CHR$(10);
#2700  IX=IX+1
2710  PC=L*PW:GOSUB1140
...
29810 CHAR 1,20,11,"LOADING CHARSET..."+LEFT$(SP$,22):BLOAD "CHARSET2.10",B0,P(BE)
...
56050  CI=0:PI=0:PB=0:PB=0:P=PI+4:SC=-1
...
############################
# LOAD MENU DATA FROM DISK #
############################
62900 CHAR 1,20,11,"LOADING MENU..."+LEFT$(SP$,26):BLOAD"MENU",U(DU),P(BE),B0
...
63916 PRINT"";
63918 PRINT"";
63920 PRINT"  "CHR$(27)"L";

63925 CHAR 1,1,1
63926 PRINT USING "=#####################################";"OWER OMPETITION ANAGER";


63999 PRINT CHR$(19)CHR$(19);:RETURN